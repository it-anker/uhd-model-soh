...
public class MediatorLayer : AbstractLayer
{
    ...
    public Position GetNextPoiOfType(Position position, int type, bool takeClosest = true,
        double radius = -1d)
    {
        IVectorLayer<VectorFeature> layer = GetLayer(type);
        return GetFor(layer, position, radius, takeClosest, vector =>
        {
            vector.VectorStructured.Data.TryGetValue("code", out object? typeAsCode);
        ...
        Func<IVectorFeature, bool> predicate)
    {
        ...
        if (takeClosest)
        {
            ...
            {
                bool isInRange = data.VectorStructured.Geometry.CoordinateEnumerable()
                    .Any(coordinate =>
    ...
    {
        List<int> typeIds = OsmFeatureCodes.TypesAsList
        ...
        double maxRadius = -1)
    {
        ...
        foreach (int type in typeIds.RandomEnumerable())
        {
            IVectorLayer<VectorFeature> layer = GetLayer(type);

            VectorFeature? result = takeClosest
            ...
            if (result != null)
            {
                if (maxRadius >= 0)
                {
                    ...
                    {
                        Point? centroid = result.VectorStructured.Geometry.Centroid;
        ...
        bool takeClosest = true, double maxRadius = -1)
    {
        ...
        List<int> types = typeIds.Where(type => OsmFeatureCodes.TypesAsList.Contains(type)).ToList();
