...
public class PedestrianUsesCarTests
{
    ...
    public void DriverEntersCarWithDifferentRanges()
    {
        ...
        TestMultimodalLayer multimodalLayer = new TestMultimodalLayer(fourNodeGraphEnv.GraphEnvironment);
        ISpatialGraphEnvironment environment = fourNodeGraphEnv.GraphEnvironment;
        TestDrivingPedestrian driver = new TestDrivingPedestrian
        {
        ...
        Golf carOnNode1 = Golf.Create(fourNodeGraphEnv.GraphEnvironment);
        ...
        Golf carOnNode2 = Golf.Create(fourNodeGraphEnv.GraphEnvironment);
    ...
    public void PedestrianDrivesCarTest()
    {
        ...
        Golf car = Golf.Create(fourNodeGraphEnv.GraphEnvironment);
        Assert.True(environment.Insert(car, fourNodeGraphEnv.Node1));
        Assert.Equal(car.Position, FourNodeGraphEnv.Node1Pos);

        TestDrivingPedestrian drivingPedestrian = new TestDrivingPedestrian
        {
            StartPosition = FourNodeGraphEnv.Node1Pos,
            Car = car
        };
        ...
        for (int tick = 0; tick < 5000 && !drivingPedestrian.GoalReached; tick++)
        {
    ...
    public void WalkToNodeDriveToNodeWalkToNodeTest()
    {
        ...
        bool carInserted = environment.Insert(car, fourNodeGraphEnv.Node2);
        Assert.True(carInserted);

        Route? myRoute = environment.FindRoute(fourNodeGraphEnv.Node1, fourNodeGraphEnv.Node2);
        ...
        for (int tick = 0; tick < 5000 && !driver.GoalReached; tick++) driver.Tick();
        ...
        Route? route = environment.FindRoute(fourNodeGraphEnv.Node2, fourNodeGraphEnv.Node3);
