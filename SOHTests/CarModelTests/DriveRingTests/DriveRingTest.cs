...
public class DriveRingTest
{
    ...
    public void TestDriveOnRing()
    {
        ...
        List<InfiniteSteeringDriver> driver = Enumerable.Range(0, 10).Select(x => x * 5).Reverse()
    ...
    public void DriveTwoRoundsAndDontSlowDownOnIntersection()
    {
        ...
        InfiniteSteeringDriver driver = new InfiniteSteeringDriver(context, 0, graph);

        const int ticks = 100;
        for (int tick = 0; tick < ticks; tick++, context.UpdateStep())
        {
            driver.Tick();
    ...
    public void VehicleStaysOnLaneWhenSwitchingEdges()
    {
        ...
        InfiniteSteeringDriver fastDriver = new InfiniteSteeringDriver(context, 0, graph, preferredLane);
